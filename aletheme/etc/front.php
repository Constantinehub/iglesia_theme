...
function ale_enqueue_styles() {
	...
	wp_register_style( 'aletheme_fontawesome', THEME_URL . '/fonts/font-awesome/css/fontawesome-all.min.css', array(), ALETHEME_THEME_VERSION, 'all');
...
//    wp_register_script( 'masonry', THEME_URL . '/js/scripts.js', array( 'jquery' ), ALETHEME_THEME_VERSION, true );
	...
	if(is_post_type_archive('events') or is_tax('events-category')) {
...
<<?php echo $tag ?> <?php comment_class(empty( $args['has_children'] ) ? '' : 'parent') ?> id="comment-<?php comment_ID() ?>">
    ...
    <?php } ?>
            ...
            <div class="comment_info cf">
                <div class="img">
                    <?php if ($args['avatar_size'] != 0) echo get_avatar( $comment, $args['avatar_size'] ); ?>
                </div>
                <div class="info">
                    <span class="name"><?php printf(__('%s','aletheme'), get_comment_author_link()) ?></span><?php if($depth == 1){ ?><span class="respond"><?php comment_reply_link(array_merge( $args, array('add_below' => $add_below, 'depth' => $depth, 'max_depth' => $args['max_depth']))) ?> <i class="fas fa-reply"></i></span><?php } ?><br />
                    <span class="time"><?php printf(get_comment_date()); ?></span>
                </div>
            </div>
